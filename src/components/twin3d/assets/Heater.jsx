...
export default function Heater({
  ...
  // Get temperature directly from rawReadings
  const tempValue = (sensorValues || []).find(
    (s) => s.asset_id === `AREA-HP-SEP:${assetId}` && s.sensor_type === 'TEMPERATURE'
  )?.value ?? 85;

  const tempPercent = Math.min(100, Math.max(0, (tempValue - 20) / 130)); // Based on 20-150 range
