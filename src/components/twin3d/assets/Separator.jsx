...
// Extract level from passed-down rawReadings
  const levelValue = (sensorValues || []).find(
    (s) => s.asset_id === `AREA-HP-SEP:${assetId}` && s.sensor_type === 'LEVEL'
  )?.value ?? 50;
  
  const levelPercent = Math.min(100, Math.max(0, levelValue)) / 100;
  ...
  return (
    ...
    >
      ...
      <mesh position={[1.85, 0.25 + (3.5 * levelPercent) / 2, 0]}>
        <boxGeometry args={[0.08, 3.5 * levelPercent, 0.28]} />
        <meshStandardMaterial
